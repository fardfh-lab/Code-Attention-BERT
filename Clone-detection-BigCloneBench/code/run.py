...
def tokenize_code(code_snippet):
  ...
  for i in tree:
    ...
    if(j[0]=="String"):
      j[1] = "str"
    lis.append(j[1][1:len(j[1])-1].strip(" "))
  return " ".join(lis)
...
def main():
    ...
    #freeze = True
    #if freeze:
    #   logger.info("Freezing wts of BERT but not for Classifier")
    #   for name, param in model.named_parameters():
    #     if 'classifier' not in name:  # classifier layer
    #       param.requires_grad = False
    #       logger.info("Freezing: " + str(name))
    ##new addition ends
    ...
    if freeze:
       logger.info("\n\nSHOWING LAYERS NOT FROZEN\n\n")
       for name, param in model.named_parameters():
         if param.requires_grad:
           logger.info("NOT FROZEN: "+ str(name))
